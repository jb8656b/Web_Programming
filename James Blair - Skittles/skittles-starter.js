...
window.addEventListener("load", init);
  ...
  function showAnswer() {
    ...
    // 3.3. Show p tag.
    var x = document.querySelectorAll("#jar .skittle.green").length;
    console.log(x);
  ...
  function fillJar() {
    ...
    //element.classList.add("skittle");
    // 4.3. (no code) What function ddo we use to append one DOM element as a "child" to a parent DOM element?
    //document.appendChild()
    for(let i = 0; i < (Math.random()*(11*9)+99); i++){ //Fills the jar at least halfway
      let newSkittle = document.createElement("div");
